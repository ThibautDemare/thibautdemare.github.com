...
toc_progress.create=function()
{
	...
	for (var main_sections_index=0;main_sections_index<main_sections.length;main_sections_index++)
	{
		...
		{
			var current_secondary_index = 1;
			for (var secondary_sections_index=1;secondary_sections_index<secondary_sections.length;secondary_sections_index++)
			{
				...
				if (title_element!=null)
				{
					...
					if(!title_element.hasAttribute('class') || title_element.getAttribute('class').indexOf('no-toc-progress')==-1){
						current_secondary_index = secondary_sections_index;
					}

					if (secondary_section.hasAttribute('data-state'))
					{
						secondary_section.setAttribute('data-state',secondary_section.getAttribute('data-state')+' toc-progress-'+main_sections_index.toString()+'-'+current_secondary_index.toString());
					}
					else
					{
						secondary_section.setAttribute('data-state','toc-progress-'+main_sections_index.toString()+'-'+current_secondary_index.toString());
					};

					if(!title_element.hasAttribute('class') || title_element.getAttribute('class').indexOf('no-toc-progress')==-1){
						var li_element=document.createElement('li');
						li_element.setAttribute('id','toc-progress-'+main_sections_index.toString()+'-'+current_secondary_index.toString());
						toc_progress_footer_secondary_inside_ul_ul.appendChild(li_element);
						var a_element=document.createElement('a');
						a_element.setAttribute('href','#/'+main_sections_index.toString()+'/'+current_secondary_index.toString());
						a_element.appendChild(document.createTextNode(title_element.textContent));
						li_element.appendChild(a_element);
						style_node.textContent=style_node.textContent+'.toc-progress-'+main_sections_index.toString()+'-'+current_secondary_index.toString()+' #toc-progress-'+main_sections_index.toString()+'-'+current_secondary_index.toString()+' {font-weight: bold;}\n';
					}
...
toc_progress.initialize=function(reducescroll,background)
{
	...
	this.background=background || '#2C508F';
